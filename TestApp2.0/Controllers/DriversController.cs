...
public class DriversController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<DriverResponseDTO>>> GetCustomerById(string driverId)
    {
        var response = await _driverService.GetDriverByIdAsync(driverId);
        if (response.StatusCode != 200)
        {
